...
class Model(nn.Module):
    def __init__(self, num_nodes, in_dim, h_dim, out_dim,num_classes, num_rels,
                num_hidden_layers=0,dropout=0.35,mmGATdropout = 0.35,out_drop = 0.65,residual = 0.12,gat_type = 'sum',data_file ='result/',final_embed='IMDB_SUM',text_item=None,img_item = None):
        ...
        self.preference_v = nn.Parameter(nn.init.xavier_normal_(torch.rand((4361, 64))))
        self.preference_t = nn.Parameter(nn.init.xavier_normal_(torch.rand((4361, 64))))
        self.build_model()
